...
extract.from.jML <- function(x, nms){
  # if(!inherits(x, "mjoint")) stop("'x' needs to be an 'mjoint' object.")
  qz <- qnorm(.975)
  co <- x$coeffs
  Omega <- setNames(c(gmvjoint:::vech(co$D), co$beta, co$sigma2, co$gamma[grepl("gamma", names(co$gamma))],
                      co$gamma[!grepl("gamma", names(co$gamma))]),
                    nms) # nms from gmvjoint for parity sake.
  # Reordering standard error vector
  SE <- x$SE
  nmSE <- names(SE)
  ind.first.gamma <- min(which(grepl("^gamma", nmSE)))
  ind.zeta <- ind.first.gamma-1L
  SE <- setNames(c(SE[-ind.zeta], SE[ind.zeta]), nms)
  ...
  if(units == "mins")
    ...
    stop(sQuote(units), " not coded.\n")
  et <- setNames(et, c("Total", "EM"))
  
  list(
    Omega = Omega, SE = SE, lb = lb, ub = ub, elapsed = et["EM"], total = et["Total"]
  )
}
