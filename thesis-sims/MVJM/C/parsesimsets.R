...
caption <- paste(sapply(seq_along(parsed), function(x){
  ...
  paste0("Median [IQR] elapsed time taken for approximate EM algorithm to converge",
         " and standard error calculation for ", gsub('\\_'," ",nm[x]), " was ", sprintf("%.3f [%.3f, %.3f] seconds", qn.et[1], qn.et[2], qn.et[3]),
         " and total computation time was ", sprintf("%.3f [%.3f, %.3f] seconds", qn.tot[1], qn.tot[2], qn.tot[3]))
}), collapse = '; ')
caption <- paste("Parameter estimates for differing censoring rates $\\Upsilon\\in\\lbr e^{-3.5}, e^{-2.6}, e^{-1.9}\\rbr$. `Mean (SD)' denotes",
...
addtorow <- list()
addtorow$pos <- list(-1)
addtorow$command <- paste0("&", paste0("\\multicolumn{5}{c}{", nm.xt, "}", collapse = '&'), '\\\\')
...
# png(save.dir.file.path("C_gammazeta.png", load.dir), width = 148, height = 90,
#     units = 'mm', res = 500, type = 'Xlib')
gam.zet %>% 
  mutate(nlab = forcats::fct_inorder(tools::toTitleCase(gsub("\\_censoring$", "", n)))) %>% 
  ...
  theme_csda("Arial") + 
  labs(x = "Censoring amount", y = "Estimate") + 
...
# dev.off()
ggsave(save.dir.file.path("C_gammazeta.png", load.dir), 
       width = 148, height = 110, units = 'mm', limitsize = F, dpi = 500*2)
