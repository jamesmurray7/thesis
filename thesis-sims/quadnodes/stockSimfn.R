...
# Heuristic ---------------------------------------------------------------
stringer.gh <- function(data, pc){
  pc <- sapply(pc, function(pc) if(pc > 1) pc <- pc/100) # safeguard 
  stopifnot(all(pc <= 1) & all(pc >= 0))
  
  ls <- unname(with(data, tapply(time, id, length)))
  n <- length(ls)
  qls <- quantile(ls, probs = pc)
  
  out <- ceiling((3/2) * log(n, base = qls) - 2)
  if(any(is.infinite(out))) out[is.infinite(out)] <- 15
  out
}
