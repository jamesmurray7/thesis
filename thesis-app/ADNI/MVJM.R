# Multivariate joint models for count data --------------------------------
# (fit by scaling)
rm(list=ls())
source('.Rprofile')
truncated <- F
...
# MVs ---------------------------------------------------------------------
pf <- parent.frame()
fit.JM <- function(resps, time.specs, Surv.formula = NULL, ...){
  family <- as.list(rep("gaussian", length(resps)))
  ...
  long.formulas <- lapply(seq_along(resps), function(k){
    ...
    form <- paste0(resps[k], "_scaled ~ ", extras, ' +', time)
...
test1 <- fit.JM(
  resps = c("ADAS13", "FAQ"), time.specs = c("linear", "quad"),
)
summary(test1)
...
a <- ROC(test1, cleandata(c("ADAS13", "FAQ")), Tstart = 2, delta = 1)
plot(a)
