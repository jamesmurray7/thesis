...
\begin{center}
    ...
    \hline 
    % Gaussian
    $\Y|\b,\sigma^2\sim N\lb\bmu,\mathrm{V}_i\rb$ & % Distribution
    {$h$: $\Exp[\Y]$} & % Link functions
    {$\Y\in\mathbb{R};$\\ $\bmu\in\mathbb{R}$} & % Domains
    {$-\frac{m_i}{2}\log2\pi-\frac{1}{2}\log|\mathrm{V}_i|$\\$-\frac{1}{2}\lb\Y-\bmu\rb^\top\mathrm{V}_i^{-1}\lb\Y-\bmu\rb$} \\ % PDF 
    % Poisson
    $\Y|\b\sim\mathrm{Po}\lb\bmu\rb$ &                                  % Distribution
    {$h$: $\log\lb\Exp[\Y]\rb$;\\$\tilde{h}$: N/A} &            % Link functions
